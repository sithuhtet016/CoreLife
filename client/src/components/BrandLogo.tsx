...
function BrandLogo({ to, onClick, className = "" }: BrandLogoProps) {
  const rootClassName = ["cl-brand-link", className].filter(Boolean).join(" ");

  return (
    <Link to={to} onClick={onClick} className={rootClassName}>
      <span className="cl-brand-icon" aria-hidden="true">
        <svg
          viewBox="0 0 512 512"
          fill="currentColor"
          className="cl-brand-icon-svg"
        >
      ...
      <span className="cl-brand-text">CoreLife</span>
    </Link>
  );
}
