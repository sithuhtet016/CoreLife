...
type PriorityIconTone = "red" | "purple" | "blue" | "indigo";
...
function AssessmentStep3Page() {
  ...
  const confidenceProgress = ((confidence - 1) / 9) * 100;
  const confidenceSliderStyle = {
    "--assessment3-confidence-progress": `${confidenceProgress}%`,
  } as CSSProperties;
  ...
  useEffect(() => {
    ...
    void getAssessmentDraft().then((draft) => {
      if (!active || !draft?.step3) return;

      if (typeof draft.step3.confidence === "number") {
        setConfidence(draft.step3.confidence);
      }

      if (
        Array.isArray(draft.step3.priorities) &&
        draft.step3.priorities.length > 0
      ) {
        setPriorities(draft.step3.priorities);
      }

      if (typeof draft.step3.timeCommitment === "string") {
        setTimeCommitment(draft.step3.timeCommitment);
      }
    });
  ...
  const handleSaveAndExit = async () => {
    setSubmitError(null);
    await saveAssessmentDraft(buildStepThreeDraft());
  ...
  const handleFinishAssessment = async () => {
    ...
    if (!isAuthenticated) {
      navigate("/register", {
        state: {
          from: "/results",
          intent: "assessment-complete" as const,
        },
      });
      return;
    }
    ...
    try {
      ...
      setSubmitError(
        error instanceof Error
          ? error.message
          : "We couldn't finish your assessment right now. Please try again.",
      );
    } finally {
            ...
            <form className="assessment3-form" action="#" method="post">
              ...
              <article className="assessment3-card">
                ...
                <div className="assessment3-priority-grid">
                  {PRIORITY_OPTIONS.map((option) => (
                    <label className="priority-checkbox" key={option.value}>
                      <input
                        type="checkbox"
                        checked={priorities.includes(option.value)}
                        onChange={(event) =>
                          togglePriority(option.value, event.target.checked)
                        }
                      />
                      <div>
                        <div className="priority-left">
                          <span
                            className={`priority-icon ${option.iconTone}`}
                            aria-hidden="true"
                          >
                            <svg
              ...
              <div className="assessment3-actions">
                ...
                <div className="assessment3-actions-right">
                  ...
                  <button
                    type="button"
                    className="assessment3-btn-primary"
              ...
              {submitError ? (
                <p className="assessment3-feedback error">{submitError}</p>
              ) : null}
            </form>
          </section>
        </section>
